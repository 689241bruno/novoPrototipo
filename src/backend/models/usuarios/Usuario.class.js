...
class Usuario {
    constructor( id, nome, email, senha, isAluno=false, isProfessor=false, isAdmin=false ) {
        this.id          = id;
        this.nome        = nome;
        this.email       = email;
        this.senha       = senha;
    ...
    static async checkUserType( email, isAluno, isProfessor, isAdmin ) {
        const [rows] = await pool.query("SELECT isAluno, isProfessor, isAdmin FROM usuarios WHERE email = ?", [email]);
        return rows[0] || null;
    }
