...
const AnimatedBarChart = ({ data, width, height }) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setCurrentValues(
        animatedValues.map((v) => parseFloat(v.__getValue().toFixed(1)))
      );
...
export default function PerfilScreen({ navigation }) {
  ...
  useEffect(() => {
    async function carregarDados() {
      try {
        ...
        const responseUsuario = await UsuarioService.buscarUsuarioPorId(
          parsedUser.id
        );
        ...
        const responseAluno = await AlunoService.buscarAlunoPorId(
          parsedUser.id
        );
        if (responseAluno?.data) {
          ...
          const conquistasTeste = [
            ...
            },
            {
              ...
              imagem: "https://cdn-icons-png.flaticon.com/512/1828/1828884.png",
            },
          ];
  ...
  const getFotoSource = (foto) => {
    if (!foto) return null;
    // Verifica se a foto é uma URL válida (que inclui URLs do Cloudinary)
    if (foto.startsWith("http") || foto.startsWith("file://")) {
      return { uri: foto };
    }
    // Caso ainda receba Base64 (temporariamente do backend antigo) ou Path local (mobile)
    // Deixo o 'data:image' por enquanto, mas o objetivo é que isso não seja mais necessário.
  ...
  const salvarEdicao = async () => {
    try {
      let fotoParaEnvio = novaFoto;

      if (novaFoto && !novaFoto.startsWith("http")) {
        console.log("Detectado nova foto. Iniciando upload para Cloudinary...");
        const cloudinaryUrl = await uploadImageToCloudinary(novaFoto);

        fotoParaEnvio = cloudinaryUrl; // fotoParaEnvio agora é a URL!

        console.log("Upload concluído. URL:", fotoParaEnvio);
      }

      // Envia a URL (fotoParaEnvio) para o backend
      await UsuarioService.editarUsuario({
        id: usuario.id,
        nome: novoNome,
        cor: novaCor,
        // O backend salva APENAS a URL de texto do Cloudinary
        foto: fotoParaEnvio,
      });

      // Atualiza o estado local e o AsyncStorage com a URL
      const atualizado = {
        ...usuario,
        nome: novoNome,
        cor: novaCor,
      ...
      // Sugestão: desative o estado de carregamento aqui (setLoading(false))
    }
  };
  ...
  if (loading) {
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={{ color: "#0b4e91", marginTop: 10 }}>
          Carregando perfil...
        </Text>
      </View>
    );
  ...
  const SessionButton = ({ sessionKey, label }) => (
    ...
    >
      <Text
        style={{
          color: activeSession === sessionKey ? "#fff" : "#333",
          fontWeight: "bold",
        }}
      >
        {label}
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
        ...
        <ScrollView contentContainerStyle={styles.scrollContainer}>
          ...
          <View
            style={[
              ...
              { backgroundColor: usuario.cor || "#9ac7ecff" },
            ]}
          >
            <TouchableOpacity
            ...
            >
              <View style={styles.avatarOuterCircle}>
                {console.log("🧠 URI da foto:", usuario.url_foto)}
                {usuario.url_foto ? (
                  <Image
                    source={getFotoSource(usuario.url_foto)}
                  ...
                  <>
                    ...
                    <Icon
                      ...
                      size={80}
                      color="#fff"
                      style={styles.avatarIconOverlay}
                    />
                  </>
                )}
              </View>
          ...
          <View style={[styles.card, { width: cardWidth }]}>
            ...
            <Text style={styles.infoText}>
              ...
              {aluno ? `#${aluno.ranking}` : "Carregando..."}
            </Text>
            <Text style={styles.infoText}>
              <Text style={styles.label}>XP:</Text>{" "}
              {aluno ? aluno.xp.toLocaleString() : "Carregando..."}
            </Text>
          </View>
          ...
          <View style={[styles.card, { width: cardWidth }]}>
            ...
            {activeSession === "atividades" && (
              <AnimatedBarChart
                data={notasAtividades}
                width={notasAtividades.labels.length * 60}
                height={200}
              />
            )}
            ...
            {activeSession === "simulados" && (
              <AnimatedBarChart
                data={notasSimulados}
                width={notasSimulados.labels.length * 60}
                height={200}
              />
            )}
            ...
            {activeSession === "redacoes" && (
              <AnimatedBarChart
                data={notasRedacoes}
                width={notasRedacoes.labels.length * 60}
                height={200}
              />
            )}
          </View>
      ...
      <Modal visible={editando} animationType="slide" transparent={false}>
        <SafeAreaView
          style={[styles.modalFullScreen, { backgroundColor: "#fff" }]}
        >
                  ...
                  if (Platform.OS === "web") {
                    ...
                    input.onchange = (e) => {
                      ...
                      if (file) {
                        ...
                        reader.onload = (event) =>
                          setNovaFoto(event.target.result);
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            ...
            <TouchableOpacity
              style={[
                styles.modalButton,
                { backgroundColor: "#0b4e91", marginTop: 15 },
              ]}
